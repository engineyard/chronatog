...
current_path = '/data/chronatog/current'
shared_path = '/data/chronatog/shared'
shared_bundler_gems_path = "/data/chronatog/shared/bundled_gems"
working_directory '/data/chronatog/current/'
...
listen '/var/run/engineyard/unicorn_chronatog.sock', :backlog => 1024
...
pid "/var/run/engineyard/unicorn_chronatog.pid"
...
before_fork do |server, worker|
  Chronatog::Server::Model.connection.disconnect!
...
after_fork do |server, worker|
  worker_pid = File.join(File.dirname(server.config[:pid]), "unicorn_worker_chronatog_#{worker.nr}.pid")
  File.open(worker_pid, "w") { |f| f.puts Process.pid }
  Chronatog::Server::Model.establish_connection(CHRONOS_DB_CREDS)
end
